...
// import CorporateSearch from "@/pages/search/corporateSearch";
// import RoundTripSearch from "@/pages/search/RoundTripSearch";
...
const OneWayTab = () => {
  ...
  const [departureDate, setDepartureDate] = useState(new Date());
  const [selectedClass, setSelectedClass] = useState(ticketClasses[0].value);
  const [searchResults, setSearchResults] = useState(null); // New state for storing results
  ...
  const searchFlight = async () => {
    ...
    try {
      ...
      // console.log(data)
      setSearchResults(data.data); // Update the results state
      // setSearchParams({
      //   departureCity,
      //   arrivalCity,
      //   departureDate,
  ...
  return (
    ...
    {searchResults && (
      <div className="search-results flex flex-col gap-3">
        <h2>Search Results</h2>
        {searchResults.length > 0 ? (
          searchResults.map((result: string, index: string) => (
