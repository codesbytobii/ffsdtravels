...
const OneWayTab = () => {
  ...
  const searchFlight = async () => {
      ...
      originDestinations: [
        {
          ...
          originLocationCode: departureCity,
          destinationLocationCode: arrivalCity,
          departureDateTimeRange: { date: formattedDepartureDate, time: "10:00:00" },
        },
      ],
      travelers: [
        ...Array.from({ length: adultCount }, (_, i) => ({
    ...
    try {
      ...
      const token = loginData.accessToken; // Assuming the API responds with { token: 'YOUR_API_TOKEN' }
      ...
      if (!response.ok) throw new Error("Error fetching flight data");
  
      const data = await response.json();
  
      // Assume flight selection logic occurs here
      const selectedFlight = data.data[0]; // Replace with actual selection logic
      const flightOfferId = selectedFlight.id; // Get the correct field for offer ID
  
      // Call the price confirmation API
      await confirmFlightPrice(flightOfferId);
    } catch (error) {
  ...
  const confirmFlightPrice = async (flightOfferId: string) => {
    const confirmUrl = "https://test.ffsdtravels.com/api/flight/price/confirm";
    try {
      ...
      if (!response.ok) throw new Error("Price confirmation failed");
      ...
      showErrorMessage("Price confirmation failed.");
      console.error(error);
    }
  };
...
//       const loginData = await loginResponse.json();
//       token = loginData.accessToken; // Assuming the API responds with { token: 'YOUR_API_TOKEN' }
...
//       console.log('flightOfferId', flightOfferId)
  
//       // Call the price confirmation API
//       await confirmFlightPrice(flightOfferId, token);
//     } catch (error) {
//       showErrorMessage("Failed to fetch flight data.");
//       console.error(error);
//     } finally {
...
//   const confirmFlightPrice = async (flightOfferId: string, token: string) => {
//     const confirmUrl = "https://test.ffsdtravels.com/api/flight/price/confirm";
//     try {
//       const response = await fetch(confirmUrl, {
//         method: "POST",
